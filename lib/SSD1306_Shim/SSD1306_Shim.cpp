...
/*!
    ...
    @return SSD1306_Shim object.
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
SSD1306_Shim::SSD1306_Shim(uint8_t w, uint8_t h, TwoWire *twi,
                                   int8_t rst_pin, uint32_t clkDuring,
                                   uint32_t clkAfter)
    : width(w), height(h), spi(NULL), wire(twi ? twi : &Wire), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
SSD1306_Shim::SSD1306_Shim(uint8_t w, uint8_t h, int8_t mosi_pin,
                                   int8_t sclk_pin, int8_t dc_pin,
                                   int8_t rst_pin, int8_t cs_pin)
    : width(w), height(h), spi(NULL), wire(NULL), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
SSD1306_Shim::SSD1306_Shim(uint8_t w, uint8_t h, SPIClass *spi,
                                   int8_t dc_pin, int8_t rst_pin, int8_t cs_pin,
                                   uint32_t bitrate)
    : width(w), height(h), spi(spi ? spi : &SPI), wire(NULL), buffer(NULL),
...
/*!
    @brief  Destructor for SSD1306_Shim object.
*/
SSD1306_Shim::~SSD1306_Shim(void) {
  if (buffer) {
    free(buffer);
    buffer = NULL;
  }
...
inline void SSD1306_Shim::SPIwrite(uint8_t d) {
  if (spi) {
    (void)spi->transfer(d);
  } else {
...
void SSD1306_Shim::ssd1306_command1(uint8_t c) {
  if (wire) { // I2C
...
void SSD1306_Shim::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  if (wire) { // I2C
    wire->beginTransmission(i2caddr);
    WIRE_WRITE((uint8_t)0x00); // Co = 0, D/C = 0
    uint8_t bytesOut = 1;
...
/*!
    ...
    @return None (void).
*/
void SSD1306_Shim::ssd1306_command(uint8_t c) {
  TRANSACTION_START
  ssd1306_command1(c);
  TRANSACTION_END
}
...
*/
bool SSD1306_Shim::begin(uint8_t vcs, uint8_t addr, bool reset,
                             bool periphBegin) {
...
*/
void SSD1306_Shim::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void SSD1306_Shim::clearDisplay(void) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void SSD1306_Shim::drawFastHLine(int16_t x, int16_t y, int16_t w,
                                     uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void SSD1306_Shim::drawFastVLine(int16_t x, int16_t __y, int16_t __h,
                                     uint16_t color) {
...
*/
bool SSD1306_Shim::getPixel(int16_t x, int16_t y) {
  if ((x >= 0) && (x < width) && (y >= 0) && (y < height)) {
    // Pixel is in-bounds. Rotate coordinates if needed.
...
*/
uint8_t *SSD1306_Shim::getBuffer(void) { return buffer; }
...
*/
void SSD1306_Shim::display(uint8_t *ptr) {
  if (ptr == NULL) {
    ptr = buffer;
  }
  TRANSACTION_START
...
void SSD1306_Shim::startscrollright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void SSD1306_Shim::startscrollleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void SSD1306_Shim::startscrolldiagright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void SSD1306_Shim::startscrolldiagleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
/*!
    ...
    @return None (void).
*/
void SSD1306_Shim::stopscroll(void) {
  TRANSACTION_START
...
*/
void SSD1306_Shim::invertDisplay(bool i) {
  TRANSACTION_START
...
*/
void SSD1306_Shim::dim(bool dim) {
